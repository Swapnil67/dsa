...
int subarraySum(vector<int> nums, int k) {
  ...
  while (j < n) {
    // * Shrink window 
    // * on duplicate element or on set size >= k
    while (i < j && (st.count(nums[j]) || st.size() >= k)) {
      cur_sum -= nums[i];
      st.erase(nums[i]);
      i++;
    }
...
int main(void) {
  ...
  // * testcase 3
  int window_size = 3;
  vector<int> arr = {1, 1, 1, 7, 8, 9};

  cout << "Distinct Elements = " << window_size << endl;
  printArr(arr);

  // int ans= bruteForce(arr, window_size);
  int ans= subarraySum(arr, window_size);
  cout << "Maximum Sum of Distinct Subarrays With Length K: " << ans << endl;
